...
export default function App() {
  ...
  useEffect(() => {
    if (!IS_MOBILE) return;
    return addBackHandlerImmediate(() => {
      // Close settings overlay first (tablet)
      if (showSettingsOverlay) {
        setShowSettingsOverlay(false);
        return true;
      }
      // Close phone settings tab
      if (isPhone && mobileNavTab !== "terminal") {
        setMobileNavTab("terminal");
        return true;
      }
      // Close phone drawers
      if (isPhone && (mobileNavTab === "files" || mobileNavTab === "source-control")) {
        setMobileNavTab("terminal");
        return true;
      }
      return false; // not consumed — let default behavior
    });
  }, [isPhone, showSettingsOverlay, mobileNavTab]);

  // Register mobile settings handler so openSettingsWindow() can show
  // MobileSettingsPanel on Android instead of trying to create a new window.
  useEffect(() => {
    if (IS_MOBILE) {
      setMobileSettingsHandler((_tab) => {
        if (isPhone) {
      ...
      return () => setMobileSettingsHandler(null);
    }
    setMobileSettingsHandler(null);
  }, [isPhone]);
  ...
  useEffect(() => {
    ...
    void onThemeEdit(async (req) => {
      ...
      openFileTab(path);
      try { void getCurrentWebviewWindow().setFocus(); } catch {}
    }).then((fn) => {
      if (alive) unsub = fn;
      else fn();
    });
      ...
      <TooltipProvider>
        <div className="relative flex h-screen flex-col overflow-hidden bg-background text-foreground">
          ...
          <main className="zoom-content flex min-h-0 flex-1 flex-col">
            {isPhone ? (
              <div className="min-h-0 flex-1">
                {mobileNavTab === "terminal" ? (
                  <div className="relative min-h-0 h-full">
                    {workspaceSurface}
                  </div>
                ) : null}
                {mobileNavTab === "ai" && hasComposer ? (
                  <MobileAiChat onClose={() => setMobileNavTab("terminal")} />
                ) : null}
                {mobileNavTab === "settings" ? (
                  <MobileSettingsPanel onClose={() => setMobileNavTab("terminal")} />
                ) : null}
              </div>
            ) : (
              <ResizablePanelGroup
          ...
          {isPhone ? (
            <>
              <MobileSidebarDrawer
                open={mobileNavTab === "files"}
                onOpenChange={(open) => {
                  if (!open) setMobileNavTab("terminal");
                }}
                title="Files"
              >
                <FileExplorer
              ...
              <MobileSourceControlDrawer
                open={mobileNavTab === "source-control"}
                onOpenChange={(open) => {
                  if (!open) setMobileNavTab("terminal");
                }}
                sourceControl={sourceControl}
          ...
          {/* Settings overlay for tablet (non-phone Android) */}
          {IS_MOBILE && !isPhone && showSettingsOverlay && (
            <div className="fixed inset-0 z-50 bg-background">
              <MobileSettingsPanel onClose={() => setShowSettingsOverlay(false)} />
            </div>
          )}
        </div>
      </TooltipProvider>
