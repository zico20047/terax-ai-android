...
impl WorkspaceRegistry {
    pub fn authorize<P: AsRef<Path>>(&self, path: P) -> std::io::Result<PathBuf> {
        ...
        // On Android, canonicalize may fail due to SELinux. Use the raw path as fallback.
        let canonical = std::fs::canonicalize(path_ref)
    ...
    pub fn canonicalize_cached<P: AsRef<Path>>(&self, path: P) -> std::io::Result<PathBuf> {
        ...
        // On Android, canonicalize may fail due to SELinux. Use raw path as fallback.
...
pub fn bootstrap_registry(registry: &WorkspaceRegistry) {
    ...
    log::info!("bootstrap_registry: launch_dir = {}", launch.display());
    let _ = registry.authorize(&launch);
    if let Some(home) = dirs::home_dir() {
        log::info!("bootstrap_registry: home_dir = {}", home.display());
        let _ = registry.authorize(&home);
    }
    // On Android, authorize shared storage so file explorer works
    #[cfg(target_os = "android")]
    {
        let storage = std::path::PathBuf::from("/storage/emulated/0");
        if storage.is_dir() {
            log::info!("bootstrap_registry: storage = {}", storage.display());
            let _ = registry.authorize(&storage);
        }
    }
}
...
pub async fn workspace_current_dir(
    ...
    let result = crate::modules::fs::to_canon(&canonical);
    log::info!("workspace_current_dir: {}", result);
    Ok(result)
}
...
fn resolve_launch_dir() -> PathBuf {
    ...
    // On Android, prefer shared storage as the default workspace
    #[cfg(target_os = "android")]
    {
